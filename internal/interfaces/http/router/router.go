...
r.engine.Use(middleware.SecureTransportDetectionMiddleware()) // Detect HTTPS
r.engine.Use(middleware.SecurityHeaders())             // Security headers third
r.engine.Use(middleware.CORS())                        // CORS fourth (after security headers)
...
admin.Use(middleware.UserRateLimiter(100.0/60.0, 100)) // 100 req/min per user
{
